...
template<class T>
bool DLList<T>::generate_Servers(int amount) {
    return false;
}
...
template<>
bool DLList<Server>::generate_Servers(int amount) {
    if(amount <= 0)
        return false;
    ...
    for(int i = 0 ; i < amount; i++){
        ...
        newServer = new Server(temp1, temp2, temp3, temp4);
        add_node(*newServer);              //adding the random string to the list
        temp1.erase();                //erasing the data from the string in order to keep it clean
        temp2.erase();                //erasing the data from the string in order to keep it clean
        temp3.erase();                //erasing the data from the string in order to keep it clean
        temp4.erase();                //erasing the data from the string in order to keep it clean
    }
    return true;
}
